...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function openMap(place){if(!place||place.lat==null||place.lng==null)return;if(!mapChoice){pendingPlace=place;overlay.classList.add('active');overlay.setAttribute('aria-hidden','false');return;}const url=mapChoice==='apple'?`https://maps.apple.com/?q=${encodeURIComponent(place.name)}&ll=${place.lat},${place.lng}`:`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(place.name+', Japan')}&center=${place.lat},${place.lng}`;window.location.href=url;}
